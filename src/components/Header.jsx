...
export default function Header({ carrito, guitar, quitaUno, vaciarCarrito, quitaGuitarra, agregarCarrito }) {
    const isCarritoVacio = useMemo(() => carrito.lenght === 0, [carrito])
    const total = useMemo(
        () => carrito
            .reduce((t, g) => t + (g.cantidad * g.precio), 0), [carrito]
    )
    return (
                          ...
                          {carrito.map((guitar, g) => (
                            <tr key={g}>
                              ...
                              <td className="flex align-items-start gap-4">
                                <button type="button" className="btn btn-dark" onClick={() => quitaUno(guitar.id)}>
                                  -
                                </button>
                                {guitar.cantidad}
                                <button type="button" className="btn btn-dark" onClick={() => agregarCarrito(guitar)}>
                                  +
                                </button>
                              </td>
                              <td>
                                <button className="btn btn-danger" type="button" onClick={() => quitaGuitarra(guitar.id)}>
                                  X
                                </button>
                              </td>
                            </tr>
